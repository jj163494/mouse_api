...
class KeyboardController extends PublicController {
	...
	public function actionDeleteResconstructproject() {
		...
		if ($Id) {
			$deleteproject = TdResconstructProject::model ()->deleteAll ( "userid='{$member_id}' and id='{$Id}' and device_code='{$deviceCode}'" );
			if ($deleteproject) {
				$result ['ret_num'] = 0;
				$result ['ret_msg'] = '操作成功';
			} else {
			...
			$deleteproject = TdResconstructProject::model ()->deleteAll ( "userid='{$member_id}' and project_name='{$projectName}' and device_code='{$deviceCode}'" );
			if ($deleteproject) {
				$result ['ret_num'] = 0;
				$result ['ret_msg'] = '操作成功';
			} else {
