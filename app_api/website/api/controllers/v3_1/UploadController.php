...
class UploadController extends PublicController {
	...
	/**
	 * @SWG\Get(
	 *     path="/app_api/website/api.php/v3_1/upload/getTrainingInfoByDate",
		...
		if(!empty($test_data_list)){
			...
			foreach($test_data_list as $k => $data_info){
				...
				$training_data[$k] = array(
					'game_name' => (!empty($data_info['game_name'])) ? $data_info['game_name'] : $this->__getTempGameName($data_info['game_type']),		// 游戏名称
			...
			$result = array (
				...
				'best_data' => array(
					// 最佳数据
					'game_name' => (!empty($best_data['game_name'])) ? $best_data['game_name'] : $this->__getTempGameName($best_data['game_type']),		// 游戏名称
	...
	/**
	 * @SWG\Post(
	 *     path="/app_api/website/api.php/v3_1/upload/getTrainingResult",
	...
	public function actionGetTrainingResult(){
		...
		if(empty($game_type) || !in_array($game_type, array(1, 2, 3, 4))){
			$result['ret_num'] = 212;
			$result['ret_msg'] = $this->language->get('wrong_type');
			echo json_encode ( $result );
			die ();
		}
		...
		if(empty($game_name)){
			$game_name = $this->__getTempGameName($game_type);
		}
		...
		$td_upload_model->game_name = $game_name;														// 游戏名称
	...
	/**
	 * @SWG\Get(
	 *     path="/app_api/website/api.php/v3_1/upload/getTrainingSummary",
	...
	/**
	 * @SWG\Get(
	 *     path="/app_api/website/api.php/v3_1/upload/getTrainingList",
		...
		if(!empty($data_list)){
			...
			foreach($data_list as $k => $training_data){
				$training_list[$k] = array(
					...
					'game_name' => (!empty($training_data['game_name'])) ? $training_data['game_name'] : $this->__getTempGameName($training_data['game_type']),		// 游戏名称
			...
			// 获取总数据数计算合计页数
			$total = $upload_model->getUploadTestDataInfo($where, "count(*) AS id");
			// 向上取整
			$total_page = ceil($total['id'] / $page_size);

			// 拼接返回数组
			$result = array(
				'ret_num' => 0,
				'ret_msg' => 'ok',
				'total_page' => $total_page,				// 合计页数
	...
	private function _get_apm_score($avg_apm, $game_type){
		...
		switch($game_type){
			...
			case 4:
				// 其他
				$apm_conf1 = ETC_APM_CONF1;
				$apm_conf2 = ETC_APM_CONF2;
				$apm_conf3 = ETC_APM_CONF3;
				$apm_conf4 = ETC_APM_CONF4;
				$apm_conf5 = ETC_APM_CONF5;
				break;
			default:
				break;
		}
	...
	private function _get_mental_info($avg_heart, $min_heart, $max_heart, $game_type, $time_long){
		...
		switch($game_type){
			...
			case 4:
				// 其他
				$heart_conf1 = ETC_HEART_CONF1;
				$heart_conf2 = ETC_HEART_CONF2;
				$heart_conf3 = ETC_HEART_CONF3;
				$heart_conf4 = ETC_HEART_CONF4;
				$heart_tfilter = ETC_HEART_TFILTER;
				break;
			default:
				break;
		}
	...
	private function _get_agi_info($avg_agi, $min_agi, $max_agi, $game_type){
		...
		switch($game_type){
			...
			case 4:
				// 其他
				$agi_conf1 = ETC_AGI_CONF1;
				$agi_conf2 = ETC_AGI_CONF2;
				break;
			default:
				break;
		}
	...
	private function _get_pro_info($apm_score, $mental_score, $agi_score, $mc_score, $md_score, $game_type, $time_long){
		...
		switch($game_type){
			...
			case 4:
				// 其他
				$pro_conf1 = ETC_PRO_CONF1;
				$pro_conf2 = ETC_PRO_CONF2;
				$pro_conf3 = ETC_PRO_CONF3;
				$pro_conf4 = ETC_PRO_CONF4;
				$pro_conf5 = ETC_PRO_CONF5;
				break;
			default:
				break;
		}
	...
	private function _get_move_count_score($move_count, $max_mc, $game_type, $time_long){
		...
		switch($game_type){
			...
			case 4:
				// 其他
				$mc_conf1 = ETC_MC_CONF1;
				$mc_conf2 = ETC_MC_CONF2;
				break;
			default:
				break;
		}
	...
	private function _get_move_distance_score($move_distance, $max_md, $game_type, $time_long){
		...
		switch($game_type){
			...
			case 4:
				// 其他
				$md_conf1 = ETC_MD_CONF1;
				$md_conf2 = ETC_MD_CONF2;
				$md_conf3 = ETC_MD_CONF3;
				break;
			default:
				break;
		}
	 ...
	 * @return string
	 */
	private function __getTempGameName($game_type){
		switch ($game_type){
			case 1:
				$game_name = "RTS";
				break;
